...
Write-Host "  ✅ GlazeWM config → $glazeDir\config.yaml"

# Bashrc
Copy-Item "bashrc" "$env:USERPROFILE\.bashrc" -Force
Write-Host "  ✅ .bashrc → $env:USERPROFILE\.bashrc"

# Git config (delta integration)
$gitconfigDest = "$env:USERPROFILE\.gitconfig"
if (Test-Path $gitconfigDest) {
    Write-Host "  ⚠️  .gitconfig already exists — merge gitconfig manually" -ForegroundColor Yellow
} else {
    Copy-Item "gitconfig" $gitconfigDest -Force
    Write-Host "  ✅ .gitconfig → $gitconfigDest"
}
